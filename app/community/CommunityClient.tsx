...
const posts = [
  { id:"stream", type:"Эфир", title:"Разговор после полуночи", text:"Карточка будущего стрима. Дата и время должны приходить из CMS; сейчас событие не анонсировано как реальное.", label:"SCHEDULE / CMS" },
  { id:"fan-art", type:"Фан-работа", title:"Архив фан-арта", text:"Место для работ сообщества с обязательным указанием автора, разрешения на публикацию и статуса материала.", label:"FAN WORK" },
  { id:"theory", type:"Обсуждение", title:"Теории вселенной", text:"Фанатские гипотезы не смешиваются с каноном и всегда отображаются как пользовательские трактовки.", label:"NOT CANON" },
];
...
export default function CommunityClient(){
  ...
  return <main className="community-page">
    ...
    <section className="community-feed"><div className="community-heading"><div><div className="ds-kicker">Feed / Community</div><h2>Поток архива</h2></div><p>Стандартно.</p></div><div className="community-post-grid">{posts.map((post)=><article className="community-post ds-panel" key={post.id}><div><small>{post.label}</small><span>{post.type}</span></div><h3>{post.title}</h3><p>{post.text}</p><button type="button" disabled>Материал появится из CMS</button></article>)}</div></section>
